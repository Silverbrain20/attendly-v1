...
const Attend: React.FC = () => {
  ...
  const triggerLocationCheck = () => {
    if (!navigator.geolocation) {
      setState('error');
      setErrorMsg('Your browser does not support Geolocation checks. Please try a different device.');
      return;
    }
      ...
      async (position) => {
        ...
        try {
          ...
          if (markRes.status === 'success') {
            setDistance(markRes.data?.distance_meters || 0);
            setState('success');
          }
        } catch (err: any) {
          setState('error');
      ...
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            setErrorMsg('Location permission denied. You must grant GPS permissions to verify check-in.');
            break;
          case error.TIMEOUT:
            setErrorMsg('GPS location query timed out. Try refreshing or moving closer to window/WiFi.');
            break;
          default:
            setErrorMsg('An error occurred while tracking device coordinates.');
        }
      },
    ...
    <div className="page-center">
      <div className="auth-card animate-slideUp" style={{ maxWidth: '460px' }}>
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          {/* State: LOCATING / VERIFYING (Radar) */}
          ...
          {state === 'error' && (
            <div style={{ padding: '2rem 0' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <button
                  onClick={() => { setState('locating'); triggerLocationCheck(); }}
